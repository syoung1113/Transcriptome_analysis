...
##        transcriptList.sort(key=lambda bla: bla[1]) #gene_id
    ...
    for i, v in t_dict.items():
        gene_id = geneIDs.get(i, i)
        #
        # geneDict에 gene_id가 없으면 새로 생성
        if gene_id not in geneDict:
            geneDict[gene_id] = {}
            #
            # 모든 샘플에 대해 geneDict 초기화
            for sample, _ in samples:
                if sample not in geneDict[gene_id]:
                    geneDict[gene_id][sample] = 0
        #
        # 현재 처리 중인 샘플에 대해 값을 추가
        geneDict[gene_id][s[0]] += v.get(s[0], 0)
